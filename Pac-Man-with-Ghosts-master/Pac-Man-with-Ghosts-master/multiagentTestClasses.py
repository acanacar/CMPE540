...
from .pacman import GameState, parseAgentArgs
...
def run(lay, layName, pac, ghosts, disp, nGames=1, name='games'):
    ...
    games = runGames(lay, pac, ghosts, disp, nGames, False, catchExceptions=True, timeout=120)
...
class EvalAgentTest(TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        agentOpts = parseAgentArgs(self.agentArgs) if self.agentArgs != '' else {}
        agent = agentType(**agentOpts)

        lay = getLayout(self.layoutName, 3)

        disp = self.question.getDisplay()

        random.seed(self.seed)
        games = runGames(lay, agent, self.ghosts, disp, self.numGames, False, catchExceptions=True, timeout=self.maxTime)
